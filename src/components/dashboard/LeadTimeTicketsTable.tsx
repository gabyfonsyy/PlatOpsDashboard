    ...
    () =>
      scoped.map((t) => ({
        ...
        cells: {
          ...
          product: `${t.product} ${meaningfulLabels(t.labels).join(" ")}`,
            ...
            visible.map((t) => (
              <tr key={t.issueKey}>
                ...
                <td className="px-4 py-3">
                  {t.product}
                  {meaningfulLabels(t.labels).length > 0 && (
                    <span className="block text-xs text-neutral-500">{meaningfulLabels(t.labels).join(", ")}</span>
                  )}
                </td>
