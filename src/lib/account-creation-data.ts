...
type BaseRow = Omit<Row, "se_work_cycles_json">;
...
export async function fetchAccountCreationTicketRows(startDate: string, endDate: string): Promise<Row[]> {
  ...
  const seWorkMap = await fetchSeWorkCyclesByIssueKey(filtered.map((r) => r.issue_key));
  return filtered.map((r) => ({ ...r, se_work_cycles_json: seWorkMap[r.issue_key] ?? null }));
}
